...
jQuery(document).ready(function ($) {
    ...
    $('body').on('click', '.forgot_password , .lwp_didnt_r_c', function (e) {
        ...
        $('p.status', this).show().text(drc_lwp.loadingmessage);
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (data.success == true) {
                    ...
                    //     document.location.href = drc_lwp.redirecturl;
                }
            }
        });
    });
    ...
    $('body').on('submit', 'form#lwp_activate', function (e) {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                // console.log('');
                // if (data.loggedin == true && drc_lwp.redirecturl) {
                //     location.replace(drc_lwp.redirecturl);
                // }
            }
        });
    });
    ...
    $('body').on('submit', 'form#lwp_enter_password', function (e) {
        console.log('act 2', drc_lwp.UserId);
        if (!$(this).valid()) return false;
    ...
    window.lwp_runTimer = function () {
        if (drc_lwp.timer && (drc_lwp.timer == '1' || drc_lwp.timer == 1)) {
            var lwp_start = drc_lwp.timer_count || 60;
